...
class SubcategoryAdminEdit {
	/**
	 *
	 */
	public static function catchTwig() {
		try {
			echo TwigLabs::loadTwig()->render( '/forum/subcategoryAdminEdit.twig', [
				'title'           => 'Administration',
				'editSubcategory' => self::editSubcategory(),
	...
	private static function editSubcategory() {
		$category = Database::getQuery( '
		SELECT id, name, content, slug, img, idCategory, freeAccess
		...
		if ( isset( $_POST['editSubcategory'] ) ) :
			...
			if ( empty( $edit->getFail() ) ):
				...
				// Delete old Avatar
				if ( ! is_null( $create->img ) && $create->img != $_FILES['editImg']['name'] && ! empty( $_FILES['editImg']['name'] ) ) :
					if ( file_exists( $_SERVER['DOCUMENT_ROOT'] . '/web/img/subcategory/' . $create->img ) ) {
						unlink( $_SERVER['DOCUMENT_ROOT'] . '/web/img/subcategory/' . $create->img );
					}
				...
				Database::getQuery( '
				UPDATE ln_forum_subcategory
				SET name = ?, slug = ?, content = ?, img = ?, idCategory = ?, freeAccess = ?
				WHERE id = ?', [
					$_POST['editName'],
					$_POST['editSlug'],
					$_POST['editContent'],
				...
				MessageFlash::setFlash( 'success', 'Sous-catégorie mise à jour' );
